...
describe('Credmark Access Key', () => {
  ...
  describe('#rewards', () => {
    it('should get rewards on burn', async () => {
      ...
      // debt = 100$ * (7 days / 30 days) / (1 cmk per $)
      expect(fromWei(await credmarkAccessKey.debt(0))).to.be.closeTo(
        BigNumber.from(100).mul(7).mul(10000).div(30).div(cmkPrice), // ~93 CMK
        1
      );
      ...
      expect(fromWei(await cmk.balanceOf(wallet.address))).to.be.closeTo(
        fromWei(
          fundAmount
            .sub(BigNumber.from(100).mul(7).mul(10000).div(30).div(cmkPrice))
            .add(BigNumber.from(sevenDays).mul(toWei(10)))
        ),
        100
      );
    });
    ...
    it('should get proportional rewards by multiplier', async () => {
      ...
      await rewardsPool.connect(admin).start(toWei(1)); // 10 CMK per second
      ...
      const debt = BigNumber.from(100).mul(7).mul(10000).div(30).div(cmkPrice);
      expect(fromWei(await cmk.balanceOf(wallet.address))).to.be.closeTo(
